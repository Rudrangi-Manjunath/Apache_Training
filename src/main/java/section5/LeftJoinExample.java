      ...
      /*  result.apply(ParDo.of(new DoFn<KV<String, CoGbkResult>, String>() {
        ...
        PCollection<String> Output = result.apply(ParDo.of(new DoFn<KV<String, CoGbkResult>, String>() {
            ...
            public void processElement(ProcessContext c) {
                ...
                System.out.println(key + " " + valObject.getAll(orderTuple) + " " + valObject.getAll(userTuple));
                for (String order : orderTable) {
                    if (userTable.iterator().hasNext()) {
                        c.output(key + "," + userTable.iterator().next() + "," + order);
                    } else {
        ...
        //save output to file
        Output.apply(TextIO.write().to("C:\\Users\\LPC\\Documents\\Synergy\\Apache\\LeftJoinOutput.csv").withNumShards(1).withSuffix(".csv"));
